  ...
  useEffect(() => {
    const handleCallback = async () => {
      ...
      try {
        ...
        // Retry logic for fetching user data (trigger should have created it)
        let userData: UserData | null = null;
        retries = 0;
        const maxUserRetries = 5; // Give trigger more time
        ...
        while (!userData && retries < maxUserRetries) {
          ...
          if (retries < maxUserRetries - 1) {
            console.log(`User not found in DB, waiting for trigger... (${retries + 1}/${maxUserRetries})`);
            await new Promise(resolve => setTimeout(resolve, 1000));
            retries++;
          } else {
            console.error("User not found after retries. Trigger may have failed.");
            console.error("User error:", userError);
            // Redirect to onboarding - they can complete setup there
        ...
        console.log("✅ Regular user, redirecting to agent dashboard");
